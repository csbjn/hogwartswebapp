...
import { PotionSearchOrder } from '../model/potion-search-order';
import { Difficulty } from '../model/potion-difficulty';
import { useNavigate } from 'react-router-dom';
import queryString from 'query-string';
import { PotionList } from './potion-list';
import { Potion } from '../model/potion';
import { PotionPagination } from './potion-pagination';


interface PotionSearchParams {
...
export const PotionSearchForm = () => {
  const [searchParams, setSearchParams] = useState<PotionSearchParams>({
    orderBy: PotionSearchOrder.NAME_ASC,
    offset: 0,
    limit: 20,
    page: 1,
  });
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setSearchParams(beforeParams => {
        if (name === "difficulty" && value === "All") {
          const { difficulty, ...leftParams } = beforeParams;
          return leftParams;
        } else {
  ...
  const handleSearch = async (resetOffAndPage = true) => {
    ...
    if (params.limit < 1 || params.limit > 100) {
      toast({
        ...
        description: 'A megjelenített bájitalszám, minimum 1 és maximum 100 lehet.',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
    ...
    try {
      const searchResults = await PotionService.searchPotions(params, authToken!!);
      setPotions(searchResults.results);
    ...
    if (resetOffAndPage) {
      ...
      console.error('Hiba a bájitalok betöltése közben:', error);
    }
  };



  const updateUrlParams = (params: PotionSearchParams) => {
  ...
  useEffect(() => {
    if(authToken) {
    const urlParams = queryString.parse(window.location.search) as unknown as PotionSearchParams;
  ...
  return (
    ...
    <Heading fontSize="xl" color="orange.900">
      <Text as="span" fontWeight="black">Bájitalok</Text>
    </Heading>
    </Box>
        ...
        <FormControl>
          ...
          <FormControl>
            ...
            >
               {Object.values(PotionSearchOrder).map(searchOrder => (
          ...
          <FormControl>
            <FormLabel htmlFor="limit">Bájitalszám:</FormLabel>
            <Select
              name="limit"
              onChange={handleChange}
              ...
              background='purple.100'
            >
              {[5, 10, 20, 40, 60, 80, 100].map(value => (
      ...
      {potions.length > 0 ? (
        ...
        <Box mt={8}>
          <Text>A keresett feltételre nincs egy találat sem a bájitalok között.</Text>
        </Box>
      )}
    </Container>
  );
